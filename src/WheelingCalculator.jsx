...
export default function WheelingCalculator() {
  ...
  const handleLeadSubmit = (e) => { e.preventDefault(); setStep("done"); };

  const isMunicipal = billingSource === "municipal";
  const tariff = TARIFFS[tariffKey];

  return (
    <div className="min-h-screen text-white font-sans" style={{ backgroundColor: "#0B2B26" }}>
      <style>{`
        ...
        <div className="bg-[#0d2e29]/90 backdrop-blur-sm border border-amber-900/40 rounded-3xl overflow-hidden glow-gold">
          ...
          {step === "hook" && (
            <div className="p-6 fade-up">
              ...
              <button disabled={!billingSource} onClick={() => setStep("data")}
          ...
          {step === "data" && (
            <div className="p-6 fade-up">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setStep("hook")} className="px-5 py-3 rounded-2xl border border-amber-900/40 text-stone-400 hover:text-stone-200 hover:border-amber-700/50 transition-colors text-sm">← Back</button>
                <button onClick={() => setStep("scorecard")}
          ...
          {step === "scorecard" && scorecard && (
            <div className="p-6 fade-up">
              ...
              <button onClick={() => setStep("data")} className="w-full text-center text-xs text-stone-600 hover:text-stone-400 mt-3 transition-colors">← Adjust my inputs</button>
            </div>
          )}
